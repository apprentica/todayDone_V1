<x-app-layout>
    ...
    <div class="card my-3">
        ...
        <table class="table table-striped">
            ...
            <tbody>
                @forelse ($dones as $dones)
                    <tr>
                        <td>{{ $dones->title }}</td>
                        <td>{{ $dones->description }}</td>
                        <td>
                            <form action="{{ route('todo.destroy', $dones->id) }}" method="post">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5">
                            <p class="text-default text-center fw-bold"> Complete tasks to get Today Done! </p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
